...
<?php
		...
		if(version_compare($jversion, '3.7.0', '>=')){
			$query = 'SELECT id, title, group_id FROM #__fields WHERE context = "com_content.article" ORDER BY title ASC';
			$customFields = acymailing_loadObjectList($query);

			if(!empty($customFields)){
				$query = 'SELECT id, title FROM #__fields_groups WHERE context = "com_content.article" ORDER BY title ASC';
		...
		if($tag->type != "title"){
			...
			if(!empty($tag->custom)){
				...
				foreach($fields as $oneField){
					...
					$replaceme = implode(', ', $oneField['values']);
					if(empty(trim($replaceme, ', '))) continue;
