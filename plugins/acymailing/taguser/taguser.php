...
class plgAcymailingTaguser extends JPlugin{
	...
	function acymailingtaguser_show(){
		...
		if(version_compare($jversion, '3.7.0', '>=')){
			$query = 'SELECT id, title FROM #__fields_groups WHERE context = "com_users.user" ORDER BY title ASC';
			...
			$query = 'SELECT id, title, group_id FROM #__fields WHERE context = "com_users.user" ORDER BY title ASC';
		...
		foreach($extractedTags as $i => $mytag){
			...
			if($mytag->type == 'extra'){
				...
				if(empty($mytag->id)){
					...
					if(empty(trim(implode(', ', $userFieldVals), ', '))){
