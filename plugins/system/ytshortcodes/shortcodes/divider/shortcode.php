...
function dividerYTShortcode($atts = null, $content = null) {
        ...
        if($yt_text !='')
        {
            $icon = '<span class="divider-text">'.$yt_text.'</span>';
			$css .="#".$id." .divider-text{margin: 0 20px;}";
        } else{
            if ($icon_divider !== '') {
				if (strpos($icon_divider, '/') !== false) {
					$icon = '<img class="divider-icon-img" src="' . yt_image_media($icon_divider) . '" alt="" />';
					$css .= "#".$id." .divider-icon-img{width:".$icon_size."px}";
				}
