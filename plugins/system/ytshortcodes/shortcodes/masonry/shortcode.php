...
function masonryYTShortcode($atts = null, $content = null) {
       ...
       // intense_add_script( 'packery' );
		...
		return $init_script . '<div id="' . $id_masonry . '" class="intense masonry' . ( !empty( $class ) ? ' ' . $class : '' ) . '">' . parse_shortcode(str_replace(array("<br/>","<br />"), " ", $content)) . '</div><div class="clearfix"></div>';
    }
?>