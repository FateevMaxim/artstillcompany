...
function popoversYTShortcode($atts = null, $content = null) {
	...
	return '<span class="yt-tooltip' . trim($atts['class']) . '" data-close="' . $atts['close'] . '" data-behavior="' . $atts['behavior'] . '" data-my="' . $position['my'] . '" data-at="' . $position['at'] . '" data-classes="' . implode(' ', $classes) . '" data-title="' . $atts['yt_title'] .
	'" title="' . ( $atts['yt_text'] ) . '">' . parse_shortcode(str_replace(array("<br/>","<br />","<p></p>"), " ", $content)) . '</span>';
}
?>