...
<i class="fa fa-check-circle"></i> <span class="text"><?php echo vmText::_('COM_VIRTUEMART_PRODUCT_AVAILABLE_DATE') .': '. JHtml::_('date', $product->product_available_date, vmText::_('DATE_FORMAT_LC4')); ?></span>
    </div>
	<?php
		} 
		else if ($stockhandle == 'risetime' and VmConfig::get('rised_availability') and empty($product->product_availability)) {
		?>	<div class="availability">
           <span class="text2"><?php echo JText::_('COM_VIRTUEMART_PRODUCT_AVAILABLE'); ?></span>
            <?php
...
else if (!empty($product_available_date)) {
    ?>    <div class="availability">
    	<p class="instock"><label>Доступность</label>:
<i class="fa fa-check-square-o"></i> <span class="text"><?php echo JText::_('COM_VIRTUEMART_PRODUCT_AVAILABLE_IN_STOCK'); ?></span>
</p>
    </div>
<?php
}
