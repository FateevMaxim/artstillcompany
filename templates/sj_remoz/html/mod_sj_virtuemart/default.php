              ...
              if($img){
                ...
                <div class="item-image">
                  <a href="<?php echo $item->link;?>" title="<?php echo $item->title ?>" <?php echo SjVirtueMartHelper::parseTarget($params->get('item_link_target')); ?>>
                    ...
                    <?php if ($item->prices['salesPrice'] != $item->prices['basePrice']) { 
                      $sale= round(($item->prices['basePrice'] - $item->prices['salesPrice']) / $item->prices['basePrice'] *100); ?>
                  ...
                  echo '<div class="ratingbox" >'; ?>
                    <div class="stars-orange" style="width:<?php echo round($rating * 14).'px'; ?>"></div>
                    <?php
                    echo '</div>';
                  }
                  ?>
                  ...
                  <div class="item-price">
                    <?php
                    if (!empty($item->prices['basePrice']) && $item->prices['salesPrice'] != $item->prices['basePrice']) {
                                 // $currency = CurrencyDisplay::getInstance( );
                     echo $currency->createPriceDiv ('basePrice', '', $item->prices, FALSE, FALSE, 1.0, TRUE);
                   } 
